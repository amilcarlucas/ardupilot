    ...
    */
    ...
    public:
        ...
        void set_state(ESCState _state) {
            fet_debug("Moving ESC.servo_ofs=%u from state=%u to state=%u", (unsigned)servo_ofs, (unsigned)state, (unsigned)_state);
            state = _state;
        };
    ...
    uint32_t _min_transmit_period_us;         ///< minimum allowed update period
